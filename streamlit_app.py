...
# Fancy styled button using CSS injection
st.markdown("""
<style>
.stButton button {
    background:#28a745;
...
# --- 🌧 Forecast Processing ---
if go:
    # 🔎 Get coordinates from user input or geolocation
    ...
    end_date = (today + timedelta(days=2)).strftime("%Y-%m-%d")  # 3-day window

    # 🌐 API data fetching (WeatherAPI + Open-Meteo)
    ...
    # --- 📊 Tabbed Output Sections (Without History Tab) ---
    tabs = st.tabs(["🌧 Forecast", "🗺 Map View", "📈 Trends", "🧭 Risk Overview", "📰 News"])

    with tabs[0]:
        # 🌧 Forecast Tab with tips and data
